...
game_server.lema = async function(io, socket, text, type) {
    ...
    let response = await fetch('http://text.fiit.stuba.sk:8080/lematizer/services/lemmatizer/lemmatize/fast', {
        method: 'POST',
        mode: 'cors',
...
game_server.disconnectUser = function (io, socket) {
    ...
    if (socket.room !== undefined) {
        ...
        if (game_server.gameRooms[socket.room].users.length === 0) {
            ...
            socket.broadcast.emit("update active", game_server.gameRooms.length);
        }
    }
}
